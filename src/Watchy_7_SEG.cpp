...
void Watchy7SEG::drawSteps(){
    ...
    if (currentTime.Hour == 0 && currentTime.Minute == 59){
      if (connectWiFi()) { // send step count to server before daily reset
        HTTPClient http;
        http.setConnectTimeout(3000); // 3 second max timeout
        String stepsURL = String(settings.serverURI) + String(settings.stepsEndpoint) + String(sensor.getCounter());
        http.begin(stepsURL.c_str());
